...
const styles = StyleSheet.create({
  ...
  colName: { width: '40%' }, colDni: { width: '25%' }, colHours: { width: '35%', textAlign: 'right' },
...
interface TimeTrackingAllPdfProps {
  data: any
  locale: string
}

// Un documento por empleado (misma maqueta que TimeTrackingPdf), precedido
// de una portada con el resumen de todos — pensado para entregar de una
// vez el registro de jornada completo de la clínica ante un requerimiento
// de la Inspección de Trabajo (art. 34.9 ET, RDL 8/2019).
export function TimeTrackingAllPdf({ data, locale }: TimeTrackingAllPdfProps) {
  const { clinic, employees = [], totalHoursAll = 0, exportHash, period } = data
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.titleBlock}>
          <View style={{ flex: 1 }}>
            <Text style={styles.titleMain}>Informe de control horario — todos los empleados</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.table}>
            <View style={styles.tHeadRow}>
              <Text style={[styles.th, styles.colName]}>Empleado</Text>
              <Text style={[styles.th, styles.colDni]}>DNI/NIE</Text>
              <Text style={[styles.th, styles.colHours]}>Total horas</Text>
            </View>
            {employees.map((e: any, i: number) => (
              <View key={e.employee?.id ?? i} style={[styles.tRow, i % 2 === 1 ? styles.tRowAlt : {}]} wrap={false}>
                <Text style={[styles.td, styles.colName]}>{e.employee?.full_name}{!e.employee?.active ? ' (inactivo)' : ''}</Text>
                <Text style={[styles.td, styles.colDni]}>{e.employee?.dni_nie}</Text>
                <Text style={[styles.td, styles.colHours]}>{Number(e.totalHours ?? 0).toFixed(2)} h</Text>
              </View>
            ))}
          </View>

          <View style={styles.totalBox}>
            <Text style={styles.totalLabel}>Total de horas (todos los empleados)</Text>
            <Text style={styles.totalValue}>{Number(totalHoursAll).toFixed(2)} h</Text>
          </View>
        </View>
      ...
      {employees.map((e: any) => (
        <Page key={e.employee?.id} size="A4" style={styles.page}>
          <View style={styles.titleBlock}>
            <View style={{ flex: 1 }}>
            ...
            <View>
              <Text style={styles.titleRight}>{e.employee?.full_name}</Text>
              <Text style={[styles.titleRight, { color: '#BFDBFE' }]}>{e.employee?.dni_nie}</Text>
              {e.employee?.role && <Text style={styles.titleRightMeta}>{e.employee.role}</Text>}
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            {e.records.length === 0 ? (
              <Text style={{ fontSize: 9, color: C.muted, padding: 8 }}>Sin fichajes en el periodo seleccionado.</Text>
            ) : (
              <View style={styles.table}>
                <View style={styles.tHeadRow}>
            ...
            <View style={styles.totalBox}>
              ...
              <Text style={styles.totalValue}>{Number(e.totalHours ?? 0).toFixed(2)} h</Text>
            </View>
          </View>
...
export async function timeTrackingAllPdfBlob(data: any, locale: string) {
  return pdf(<TimeTrackingAllPdf data={data} locale={locale} />).toBlob()
}
