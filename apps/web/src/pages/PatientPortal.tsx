    ...
    <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-[#1A2B4A] to-[#2563EB] px-6 py-5 text-white">
          ...
          <div className="text-xs text-white/70 mt-0.5">{t('auth.portal_subtitle')}</div>
        </div>
        <div className="px-6 py-6">
          ...
          {step === 'auth' && (
            <div className="flex flex-col gap-4">
              <h2 className="text-lg font-bold text-slate-800">{t('auth.portal_title')}</h2>
              <p className="text-sm text-slate-500">{t('auth.portal_email_prompt')}</p>
              {sent ? (
                <div className="text-center text-emerald-600 py-4">
                  <p className="font-medium">{t('auth.portal_link_sent_title')}</p>
                  <p className="text-sm mt-1">{t('auth.portal_link_sent_message')}</p>
                </div>
              ) : (
                <form onSubmit={handleRequestLink} className="flex flex-col gap-3">
                  <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('auth.portal_email_placeholder')} required
          ...
          {step === 'review' && consent && (
            <div className="flex flex-col gap-4">
              <h2 className="text-lg font-bold text-slate-800">{content?.title ?? t('auth.portal_consent_default_title')}</h2>
              {legal && (
              ...
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="text-sm text-slate-700">{t('auth.portal_accept_label')}</span>
              </label>
          ...
          {step === 'sign' && (
            <div className="flex flex-col gap-4">
              ...
              {saving && <p className="text-xs text-center text-slate-400">{t('auth.portal_saving_signature')}</p>}
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="text-center py-8 flex flex-col items-center gap-4">
              ...
              <h3 className="text-xl font-bold text-slate-800">{t('auth.portal_signed_title')}</h3>
              <p className="text-sm text-slate-500">{t('auth.portal_signed_message')}</p>
          ...
          {step === 'error' && (
            <div className="text-center py-8 text-red-500">
              <p className="font-medium">{t('verifyConsent.not_found_title')}</p>
              <p className="text-sm mt-1 text-slate-400">{t('auth.portal_error_message')}</p>
            </div>
          )}
        </div>
      </div>
