...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const { full_name, date_of_birth, id_document, id_doc_type, phone, email, address, allergies, medications, blood_type } = req.body
    const data = await queryOne(
      ...
      [clinicRow?.clinic_id, full_name, date_of_birth, id_document, id_doc_type, phone, email, address, allergies, medications, blood_type]
    )
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const data = await queryOne(
      ...
      [full_name, date_of_birth, id_document, id_doc_type, phone, email, address, allergies, medications, blood_type, id]
    )
    return res.json(data)
  } catch (err: any) { return res.status(500).json({ error: err.message }) }
})
