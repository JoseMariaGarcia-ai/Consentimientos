...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 2, borderBottomColor: "#1A2B4A",
  ...
  sectionTitle: { fontFamily: "Helvetica-Bold", fontSize: 9, color: "#1A2B4A", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 4, borderBottomWidth: 0.5, borderBottomColor: "#CBD5E1", paddingBottom: 2 },
  row: { flexDirection: "row", gap: 4, marginBottom: 2 },
  label: { fontFamily: "Helvetica-Bold", fontSize: 9, color: "#475569", width: 80 },
  value: { fontSize: 9, color: "#0F172A", flex: 1 },
  bodyText: { fontSize: 9, color: "#334155", lineHeight: 1.6 },
  legalBox: { backgroundColor: "#F1F5F9", borderLeftWidth: 3, borderLeftColor: "#2563EB", padding: 8, marginBottom: 8 },
  legalText: { fontSize: 8, color: "#475569", lineHeight: 1.5 },
  signaturesRow: { flexDirection: "row", gap: 12, marginTop: 10 },
  signatureBox: { flex: 1, borderWidth: 1, borderColor: "#CBD5E1", padding: 8, minHeight: 70, alignItems: "center" },
  signatureLabel: { fontFamily: "Helvetica-Bold", fontSize: 8, color: "#475569", marginBottom: 4 },
  qrSection: { flexDirection: "row", justifyContent: "flex-end", alignItems: "center", marginTop: 12, gap: 10 },
  ...
  footer: {
    ...
    fontSize: 7, color: "#94A3B8", flexDirection: "row", justifyContent: "space-between",
  },
...
export function ConsentPdf({
  consent, patient, doctor, clinic, language, documentHash, consentUuid, legalData, qrDataUrl,
}: ConsentPdfProps) {
  const content =
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={styles.subtitle}>{clinic?.name ?? ""}{clinic?.address ? ` · ${clinic.address}` : ''}</Text>
          </View>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{patientName}{patient?.id_document ? ` · ${patient.idDocType ?? patient.id_doc_type ?? ''} ${patient.idDocument ?? patient.id_document}` : ''}</Text>
          </View>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>{doctorName}{doctor?.specialty ? ` · ${doctor.specialty}` : ''}</Text>
          </View>
          <View style={styles.row}>
          ...
          <View style={styles.signaturesRow}>
            <View style={styles.signatureBox}>
              ...
              {consent.doctor_signature_data_url ? (
                ...
                <Text style={{ fontSize: 8, color: "#CBD5E1", marginTop: 16 }}>Sin firma</Text>
              )}
            ...
            <View style={styles.signatureBox}>
              ...
              <Text style={{ fontSize: 7, color: "#64748B", marginTop: 4 }}>{patientName} · {signedDate}</Text>
            </View>
          </View>
        </View>
        ...
        {qrDataUrl && (
          <View style={styles.qrSection}>
            <View>
              <Text style={styles.qrCaption}>Escanea para verificar{'\n'}la autenticidad de este{'\n'}consentimiento</Text>
